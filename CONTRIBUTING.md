...
BurnBar is more than the macOS app:

| Area | Path | Notes |
|------|------|--------|
| macOS app | `AgentLens/` | Menu bar UI, GRDB, parsers, dashboard |
| Shared contracts | `BurnBarCore/` | App ↔ daemon wire types |
| Daemon + CLI | `BurnBarDaemon/` | JSON-RPC daemon, `BurnBarCLI`, mission control, runs |
| Editor extension | `extensions/burnbar/` | Cursor / VS Code |
| MCP helper (optional) | `tools/burnbar-mcp/` | Read-only SQLite bridge for MCP clients |

Canonical architecture: [docs/BURNBAR_RELEASE_ARCHITECTURE.md](docs/BURNBAR_RELEASE_ARCHITECTURE.md).  
...
- **Xcode project** is generated from **`project.yml`** (XcodeGen). After editing `project.yml`, run `xcodegen generate` if you maintain `BurnBar.xcodeproj` locally.
- **Swift packages**: `swift test --package-path BurnBarCore`, `swift test --package-path BurnBarDaemon`.
- **App tests**: `./scripts/test-burnbar-app.sh` runs **`BurnBarTests` only** — see [AgentLensTests/README.md](AgentLensTests/README.md) for which sources are active vs parked.
