...
import type { BurnBarState } from '../src/state/controller';
...
describe('BurnBarRunTreeItem', () => {
  ...
  it('should set context value for daemon source', () => {
    ...
    expect(item.contextValue).toBe('burnbar.run.daemon');
  });
  ...
  it('should set context value for projected source', () => {
    ...
    expect(item.contextValue).toBe('burnbar.run.projected');
  });
...
describe('BurnBarRunListTreeDataProvider', () => {
  let mockController: BurnBarExtensionController;
  let provider: BurnBarRunListTreeDataProvider;
  ...
  beforeEach(() => {
    ...
    provider = new BurnBarRunListTreeDataProvider(mockController);
  });
  ...
  describe('getTreeItem', () => {
    it('should return the tree item as-is', () => {
      const run = createMockRun({ title: 'Test Run' });
      const treeItem = new BurnBarRunTreeItem(run);
  ...
  describe('getChildren', () => {
    ...
    it('should return runs from controller snapshot', async () => {
      ...
      children.forEach(item => {
        expect(item).toBeInstanceOf(BurnBarRunTreeItem);
      });
    });
...
describe('Run List View Integration', () => {
  ...
  it('should handle mixed run sources', () => {
    ...
    const items = runs.map(run => new BurnBarRunTreeItem(run));

    expect(items[0].contextValue).toBe('burnbar.run.daemon');
    expect(items[1].contextValue).toBe('burnbar.run.projected');
  });
});
...
function createMockState(runs: BurnBarRunProjection[]): BurnBarState {
  return {
    connected: true,
