...
import { BurnBarWorkspaceCompanion } from "../src/workspace/companion";
import { BurnBarWorkspaceRpcClient } from "../src/workspace/rpc";
import { BurnBarWorkspaceRpcError, type BurnBarWorkspaceRpcRequest } from "../src/workspace/types";
...
describe("BurnBar workspace RPC", () => {
  beforeEach(() => {
  ...
  it("gates unsafe tools while the workspace is untrusted", async () => {
    ...
    ).rejects.toMatchObject<BurnBarWorkspaceRpcError>({
      code: "TRUST_REQUIRED"
    });

    await expect(client.runTerminal({ command: "npm test" })).rejects.toMatchObject<BurnBarWorkspaceRpcError>({
      code: "TRUST_REQUIRED"
    });
