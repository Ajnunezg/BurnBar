...
import {
  ...
  activateBurnBarWorkspaceCompanion,
  type BurnBarWorkspaceCompanionOptions,
...
const mockApi = {
  ...
  createRange: vi.fn(() => ({})),
  createTerminal: vi.fn(() => ({ name: 'BurnBar', show: vi.fn(), sendText: vi.fn() })),
...
describe('BurnBarWorkspaceCompanion', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('constructor', () => {
    ...
    it('should create API on initialization', () => {
      ...
      const companion = new BurnBarWorkspaceCompanion(mockApi);
  ...
  describe('resolveWorkspaceUri integration', () => {
    it('should provide API that can resolve URIs', () => {
      const companion = activateBurnBarWorkspaceCompanion('ui');
...
describe('activateBurnBarWorkspaceCompanion', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should return a companion instance', () => {
    ...
    expect(companion).toBeInstanceOf(BurnBarWorkspaceCompanion);
  });

  it('should create companion with workspace host kind', () => {
    const companion = activateBurnBarWorkspaceCompanion('workspace');
  ...
  it('should create workspace API on activation', () => {
    const createApiSpy = vi.spyOn(api, 'createBurnBarWorkspaceApi');
    activateBurnBarWorkspaceCompanion('ui');

    expect(createApiSpy).toHaveBeenCalledWith('ui');
  });

  it('should allow multiple activations', () => {
    const companion1 = activateBurnBarWorkspaceCompanion('ui');
    const companion2 = activateBurnBarWorkspaceCompanion('workspace');

    expect(companion1).toBeInstanceOf(BurnBarWorkspaceCompanion);
    expect(companion2).toBeInstanceOf(BurnBarWorkspaceCompanion);
    expect(companion1).not.toBe(companion2);
  });

  it('should create independent API instances', () => {
    const companion1 = activateBurnBarWorkspaceCompanion('ui');
    const companion2 = activateBurnBarWorkspaceCompanion('ui');

    expect(companion1.api).not.toBe(companion2.api);
  });
});

describe('BurnBarWorkspaceCompanionOptions', () => {
  it('should accept hostKind option', () => {
    // Options type is used for configuration
    const options: BurnBarWorkspaceCompanionOptions = { hostKind: 'ui' };
    expect(options.hostKind).toBe('ui');
  });

  it('should accept workspace host kind', () => {
    const options: BurnBarWorkspaceCompanionOptions = { hostKind: 'workspace' };
    expect(options.hostKind).toBe('workspace');
  });

  it('should allow undefined options', () => {
    const options: BurnBarWorkspaceCompanionOptions | undefined = undefined;
...
describe('Companion Integration Tests', () => {
  ...
  it('should handle multiple companions with different host kinds', () => {
    const uiCompanion = activateBurnBarWorkspaceCompanion('ui');
    const workspaceCompanion = activateBurnBarWorkspaceCompanion('workspace');
...
describe('Companion Lifecycle', () => {
  ...
  it('should support multiple companion instances', () => {
    const companions = Array.from({ length: 5 }, (_, i) =>
      activateBurnBarWorkspaceCompanion(i % 2 === 0 ? 'ui' : 'workspace')
    );

    expect(companions).toHaveLength(5);
    companions.forEach(c => expect(c).toBeInstanceOf(BurnBarWorkspaceCompanion));
  });
  ...
  it('should maintain independent state per instance', () => {
    ...
    const companion1 = new BurnBarWorkspaceCompanion(api1);
    const companion2 = new BurnBarWorkspaceCompanion(api2);
...
describe('Companion Type Safety', () => {
  it('should enforce hostKind type', () => {
    ...
    const uiCompanion = new BurnBarWorkspaceCompanion(uiApi);
    const wsCompanion = new BurnBarWorkspaceCompanion(wsApi);
