...
import type { BurnBarDaemonClientLike } from '../daemon/client';
import type { BurnBarRepairResult, BurnBarRepairServiceLike } from '../daemon/repair';
import {
  BURNBAR_PROTOCOL_VERSION,
  type BurnBarApprovalDecision,
  type BurnBarApprovalRequest,
...
import type { BurnBarWorkspaceRpcClientLike } from '../workspace/rpc';
import { BurnBarWorkspaceRpcError } from '../workspace/types';
import {
...
export interface BurnBarControllerDependencies {
  client: BurnBarDaemonClientLike;
  repairService: BurnBarRepairServiceLike;
  workspaceClient: BurnBarWorkspaceRpcClientLike;
}

export interface BurnBarControllerOptions {
  clientID: string;
  sessionID?: string;
  clientName?: string;
...
export class BurnBarExtensionController {
  ...
  constructor(
    private readonly dependencies: BurnBarControllerDependencies,
    options: BurnBarControllerOptions = { clientID: randomUUID() }
  ) {
    this.clientID = options.clientID;
    this.sessionID = options.sessionID ?? randomUUID();
    this.clientName = options.clientName ?? 'BurnBar VS Code Extension';
  ...
  private async performRefresh(): Promise<void> {
    ...
    try {
      ...
      this.patchState({
        workspace: undefined,
        workspaceError: error instanceof Error ? error.message : 'BurnBar workspace companion is unavailable.'
      });
    }
    ...
    try {
      ...
      try {
        ...
        lastError = error instanceof Error ? error.message : 'BurnBar could not attach the sidebar session.';
        daemonRuns = [];
        pendingToolCalls = [];
      }
      ...
      this.patchState({
        ...
        recentUsage: [],
        catalog: undefined,
        lastError: error instanceof Error ? error.message : 'Unable to reach the BurnBar daemon.',
        runError: undefined,
        lastUpdatedAt: new Date().toISOString()
      });
    }
  ...
  async repairDaemon(): Promise<BurnBarRepairResult> {
    this.patchState({
    ...
    try {
      ...
      this.patchState({
        connectionStatus: 'disconnected',
        clientAttached: false,
        lastError: error instanceof Error ? error.message : 'BurnBar daemon repair failed.'
      });
      throw error;
    }
  }
  ...
  approvalForRun(runId: string): BurnBarApprovalRequest | undefined {
  ...
  private async ensureClientAttachment(): Promise<void> {
    ...
    if (!this.state.clientAttached) {
      throw new Error(this.state.lastError ?? 'BurnBar is not attached to the local daemon.');
    }
  }
  ...
  private async attachClientSession(): Promise<void> {
    ...
    if (response.attachedClientID !== this.clientID) {
      throw new Error(`BurnBar attached unexpected client '${response.attachedClientID}'.`);
    }
  ...
  private patchState(partial: Partial<BurnBarState>): void {
    const nextState: BurnBarState = {
      ...this.state,
      ...partial
    };
...
function mapToolError(error: unknown, tool: BurnBarToolKind): BurnBarToolExecutionError {
  ...
  const workspaceCode = error instanceof BurnBarWorkspaceRpcError ? error.code : undefined;
