/**
 * BurnBar Webview Panel
 *
 * Handles the main webview lifecycle for the BurnBar sidebar panel.
 */
...
/**
 * Webview panel for BurnBar workspace operations.
 */
export class BurnBarPanel {
  ...
  private handleWebviewMessage(message: { type: string; payload?: unknown }): void {
    ...
    case 'workspace.requestState':
      vscode.commands.executeCommand('burnbar.requestState');
      break;

    case 'workspace.selectRun': {
      const payload = message.payload as { runId: string };
      vscode.commands.executeCommand('burnbar.selectRun', payload.runId);
      break;
    }

    case 'workspace.viewRun': {
      const payload = message.payload as { runId: string };
      vscode.commands.executeCommand('burnbar.viewRun', payload.runId);
      break;
    }

    case 'workspace.cancelRun': {
      const payload = message.payload as { runId: string };
      vscode.commands.executeCommand('burnbar.cancelRun', payload.runId);
      break;
    }

    case 'workspace.retryRun': {
      const payload = message.payload as { runId: string };
      vscode.commands.executeCommand('burnbar.retryRun', payload.runId);
      break;
    }

    case 'workspace.executeTool': {
      const payload = message.payload as { tool: string; args: unknown };
      vscode.commands.executeCommand('burnbar.executeTool', payload.tool, payload.args);
      break;
    }
...
<body>
  <div class="burnbar-app">
    <header class="burnbar-header">
      <h1>BurnBar</h1>
    ...
    vscode.window.registerWebviewViewProvider(
      ...
      new (class implements vscode.WebviewViewProvider {
        resolveWebviewView(
          ...
          const panel = new BurnBarPanel(context.extensionUri, {
