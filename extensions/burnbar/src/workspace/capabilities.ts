...
function explainWorkspaceCapabilities(capabilities: {
  ...
  if (!capabilities.hasWorkspace) {
    return 'Open a workspace folder to enable BurnBar file, search, edit, and terminal tools.';
  }
  ...
  if (capabilities.untrustedWorkspace) {
    segments.push('This workspace is in restricted mode, so BurnBar will not apply patches or run terminal commands until you trust it.');
  }

  if (capabilities.readonlyWorkspace) {
    segments.push('The workspace filesystem is read-only, so BurnBar can read and search files but cannot write edits.');
  }

  if (capabilities.virtualWorkspace) {
    segments.push('The workspace uses a virtual filesystem, so terminal execution is unavailable from BurnBar.');
  }
