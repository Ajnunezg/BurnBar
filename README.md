...
| Tier | Surfaces | Notes |
|------|----------|--------|
| **Core** | macOS app (`AgentLens/`), `BurnBarCore`, local daemon (`BurnBarDaemon/`), Cursor/VS Code extension (`extensions/burnbar/`), `BurnBarCLI` | Built and exercised in CI where configured; local-first + daemon RPC are the product spine. |
...
| **Adjacent tooling** | [`tools/burnbar-mcp/`](tools/burnbar-mcp/README.md) (read-only SQLite MCP helper) | Developer convenience; not required to run BurnBar. |
| **Parked** | Older `AgentLensTests/*.swift` not listed in `project.yml` | Present in-repo for future revival; **not compiled** in the active `BurnBarTests` bundle — see [AgentLensTests/README.md](AgentLensTests/README.md) and [CONTRIBUTING.md](CONTRIBUTING.md). |
...
- `scripts/test-burnbar-app.sh` — Xcode `BurnBarTests` only (see [AgentLensTests/README.md](AgentLensTests/README.md): active sources are **`AgentLensTests.swift`** + **`BurnBarSearchIntegrationHarness.swift`**; other app XCTest files are **parked** until brought back in sync)
