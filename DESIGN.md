...
- BurnBar assistant bubbles (Local Index mode): `ember` stroke (unchanged)
...
- The retrieval pipeline injects BurnBar data as system prompt context in BOTH modes.
...
- BurnBar retrieval context injected as system prompt augmentation (same `ContextBuilder` pipeline)
...
Through BurnBar's retrieval pipeline (injected as system prompt context):
