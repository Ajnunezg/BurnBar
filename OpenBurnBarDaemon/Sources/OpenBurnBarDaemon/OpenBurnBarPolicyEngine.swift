...
public struct BurnBarPolicyEngine {
    ...
    private func defaultApprovalMessage(for intent: BurnBarAgentIntent, tool: BurnBarToolKind) -> String {
        switch tool {
        case .readFile:
            return "OpenBurnBar needs approval before reading additional workspace files for \(intent.summary.lowercased())."
        case .searchWorkspace:
            return "OpenBurnBar needs approval before searching the workspace for additional context."
        case .applyPatch:
