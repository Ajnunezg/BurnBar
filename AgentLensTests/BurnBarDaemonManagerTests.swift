...
final class BurnBarDaemonManagerTests: XCTestCase {
    @MainActor
    func test_managerPrefersDaemonRPCForConfigAndRecentUsage() async throws {
        ...
        let manager = BurnBarDaemonManager(
            paths: harness.paths,
            dependencies: BurnBarDaemonDependencies(
                fileManager: .default,
                runProcess: { _, _ in "" },
            ...
            usageSyncService: BurnBarDaemonUsageSyncService(paths: harness.paths, fileManager: .default)
        )
        ...
        XCTAssertEqual(manager.controllerProjects.first?.projectSlug, "burnbar")
    ...
    func test_installAndUninstallManageLaunchAgentAndBinary() async throws {
        ...
        let sourceBinaryURL = harness.rootURL.appendingPathComponent("source-burnbar-daemon", isDirectory: false)
        ...
        XCTAssertTrue(
            launchctlCalls.contains(where: { $0.starts(with: ["kickstart", "-k", "gui/\(getuid())/\(BurnBarDaemonRuntimePaths.launchAgentLabel)"]) })
        )
    ...
    func test_usageSync_readsProviderConfigurationSnapshot() throws {
        ...
        let service = BurnBarDaemonUsageSyncService(paths: harness.paths, fileManager: .default)
    ...
    func test_usageSync_importsDaemonUsageIntoLocalShape() throws {
        ...
        XCTAssertEqual(inserted.first?.projectName, "BurnBar Daemon")
    ...
    func test_appToDaemonHealthSmoke() async throws {
        ...
        let socketURL = URL(fileURLWithPath: "/tmp/burnbar-daemon-smoke-\(UUID().uuidString).sock")
        ...
        while Date() < deadline {
            if let response = try? BurnBarDaemonSocketClient.health(at: socketURL) {
        ...
        XCTFail("Timed out waiting for BurnBarDaemon health response")
    }
    ...
    private func daemonExecutableURL() throws -> URL {
        ...
        let daemonURL = productsURL.appendingPathComponent("BurnBarDaemon", isDirectory: false)
        guard FileManager.default.isExecutableFile(atPath: daemonURL.path) else {
            throw XCTSkip("BurnBarDaemon executable is not available in build products")
        }
        return daemonURL
    }
    ...
    private func makeRuntimePathsHarness(name: String) throws -> RuntimePathsHarness {
        ...
        let paths = BurnBarDaemonRuntimePaths(
            ...
            socketURL: rootURL.appendingPathComponent("burnbar-daemon.sock", isDirectory: false),
            logURL: daemonDirectory.appendingPathComponent("burnbar-daemon.log", isDirectory: false),
            launchAgentPlistURL: daemonDirectory.appendingPathComponent("com.burnbar.daemon.plist", isDirectory: false)
        )
    ...
    func test_resourceBundleResolverFindsBundleInSiblingResourcesDirectory() throws {
        ...
        let fakeAppBundleURL = productsURL.appendingPathComponent("BurnBar.app", isDirectory: true)
        ...
        let resolved = BurnBarDaemonBinaryResolver.resolveResourceBundle(
