...
@MainActor
final class BurnBarSearchIntegrationHarnessTests: XCTestCase {
    ...
    func test_fileRootFixtures_discoverOnlyRegisteredRoots() throws {
        let harness = try BurnBarSearchIntegrationHarness(name: "discovery-fixtures")
        defer { harness.cleanup() }
    ...
    func test_queueHelpers_projectConversationSkillAndSharedArtifacts() async throws {
        let harness = try BurnBarSearchIntegrationHarness(name: "queue-roundtrip")
        defer { harness.cleanup() }
    ...
    func test_degradedStateAssertions_detectSemanticFailures() throws {
        let harness = try BurnBarSearchIntegrationHarness(name: "degraded-assertions")
        defer { harness.cleanup() }
    ...
    func test_retrievalPerf_queryLatency_guardrails_withWarmCorpus() async throws {
        let harness = try BurnBarSearchIntegrationHarness(name: "query-latency-perf")
        defer { harness.cleanup() }
    ...
    func test_semanticPerf_annCandidateGeneration_andExactRerank_metricsGuardrails() async throws {
        let harness = try BurnBarSearchIntegrationHarness(name: "semantic-ann-perf")
        defer { harness.cleanup() }
    ...
    func test_longArtifacts_memoryAndChunking_remainBounded() async throws {
        let harness = try BurnBarSearchIntegrationHarness(name: "memory-long-artifacts")
        defer { harness.cleanup() }
    ...
    func test_runBurnBarQuery_aggregateCountsExplicitProfanityWithinLastWeek() async throws {
        ...
        let recentConversation = BurnBarSearchFixtureBuilder.conversation(
            id: "conv-agg-fuck-recent",
            fullText: "fuck fuck week",
            indexedAt: base.addingTimeInterval(-2 * 86_400)
        )
        let oldConversation = BurnBarSearchFixtureBuilder.conversation(
