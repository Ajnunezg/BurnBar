...
final class ProviderQuotaServiceTests: XCTestCase {
    ...
    func test_claudeRefresh_usesLocalBridgeSnapshotEvenWhenAPIBillingOverrideDetected() async throws {
        let home = try makeTemporaryDirectory()
        let appSupport = try makeTemporaryDirectory()
        let snapshotURL = BurnBarAppPaths(applicationSupportRoot: appSupport).claudeStatuslineSnapshotURL
        ...
        try FileManager.default.createDirectory(at: claudeDirectory, withIntermediateDirectories: true)
        let settingsURL = claudeDirectory.appendingPathComponent("settings.json")
        let wrapperPath = BurnBarAppPaths(applicationSupportRoot: appSupport).claudeStatuslineBridgeScriptURL.path
        let settings = """
        {
          "statusLine": {
            "type": "command",
