    ...
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.md) {
                ...
                Text("Only the engines you enable here appear in the dashboard and menu bar chat header. Turn on each one you actually use so BurnBar does not list every provider.")
                ...
                Text("Leave the field below empty unless you set API_SERVER_KEY in ~/.hermes/.env — then paste the same value here so BurnBar can connect.")
                    .font(DesignSystem.Typography.caption)
                    .foregroundStyle(DesignSystem.Colors.textSecondary)
                ...
                Text("Optional chat model id for the gateway (same as the JSON `model` field). Leave empty to let BurnBar choose: if the gateway lists MiniMax but you use Codex with a ChatGPT account, BurnBar sends a Codex-supported model instead (e.g. gpt-5.4-mini). Set only if you need a specific id.")
                    .font(DesignSystem.Typography.caption)
                    .foregroundStyle(DesignSystem.Colors.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
