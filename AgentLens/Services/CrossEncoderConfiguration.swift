...
enum CrossEncoderProviderID: String, CaseIterable, Codable, Identifiable {
    ...
    var requirementDescription: String {
        ...
        case .minimax:
            return "Requires a MiniMax API key in BurnBar’s provider settings."
        case .zai:
            return "Requires a Z.ai API key in BurnBar’s provider settings."
        case .openrouter:
            return "Requires an OpenRouter API key in BurnBar’s provider settings."
        case .hermes:
            return "Hermes gateway on `http://localhost:8642` — enable API_SERVER_ENABLED in ~/.hermes/.env, run hermes gateway run. Token in BurnBar only if you set API_SERVER_KEY in that file."
        }
    }
}
