...
final class ProjectionStore {
    ...
    func schemaInventory() throws -> LocalSearchSchemaInventory {
        ...
        return try dbQueue.read { db in
            let tables = try String.fetchAll(
                ...
                SELECT name
                FROM sqlite_master
                WHERE type = 'table' AND name IN (\(BurnBarDatabase.sqlPlaceholders(count: expectedTables.count)))
                ORDER BY name ASC
                """,
            ...
            let indexes = try String.fetchAll(
                ...
                SELECT name
                FROM sqlite_master
                WHERE type = 'index' AND name IN (\(BurnBarDatabase.sqlPlaceholders(count: expectedIndexes.count)))
                ORDER BY name ASC
                """,
    ...
    static func projectionJob(from row: Row) -> ProjectionJobRecord? {
        ...
        let scheduledAt = BurnBarDatabase.parseDateValue(row["scheduledAt"]) ?? Date()
        let availableAt = BurnBarDatabase.parseDateValue(row["availableAt"]) ?? scheduledAt
        let createdAt = BurnBarDatabase.parseDateValue(row["createdAt"]) ?? scheduledAt
        ...
        return ProjectionJobRecord(
            ...
            startedAt: BurnBarDatabase.parseDateValue(row["startedAt"]),
            completedAt: BurnBarDatabase.parseDateValue(row["completedAt"]),
            leaseOwner: row["leaseOwner"] as? String,
            leaseExpiresAt: BurnBarDatabase.parseDateValue(row["leaseExpiresAt"]),
            createdAt: createdAt,
            updatedAt: updatedAt
        )
    }
    ...
    static func retrievalHealth(from row: Row) -> RetrievalHealthRecord? {
        ...
        let observedAt = BurnBarDatabase.parseDateValue(row["observedAt"]) ?? Date()
        let updatedAt = BurnBarDatabase.parseDateValue(row["updatedAt"]) ?? observedAt
