...
final class ArtifactStore {
    ...
    func fetchSourceArtifacts(
        ...
        clauses.append("sourceKind IN (\(BurnBarDatabase.sqlPlaceholders(count: kindValues.count)))")
        args.append(contentsOf: kindValues)

        if normalizedRoots.isEmpty == false {
            clauses.append("rootPath IN (\(BurnBarDatabase.sqlPlaceholders(count: normalizedRoots.count)))")
    ...
    func fetchSharedArtifactSyncStates(
        ...
        if let statuses, statuses.isEmpty == false {
            clauses.append("syncStatus IN (\(BurnBarDatabase.sqlPlaceholders(count: statuses.count)))")
    ...
    func fetchSharedArtifactAuditEvents(
        ...
        if let actions, actions.isEmpty == false {
            clauses.append("action IN (\(BurnBarDatabase.sqlPlaceholders(count: actions.count)))")
    ...
    static func sourceArtifact(from row: Row) -> SourceArtifactRecord? {
        ...
        let discoveredAt = BurnBarDatabase.parseDateValue(row["discoveredAt"]) ?? Date()
        let createdAt = BurnBarDatabase.parseDateValue(row["createdAt"]) ?? discoveredAt
        let updatedAt = BurnBarDatabase.parseDateValue(row["updatedAt"]) ?? createdAt
        let deletedAt = BurnBarDatabase.parseDateValue(row["deletedAt"])
        ...
        return SourceArtifactRecord(
            ...
            fileModifiedAt: BurnBarDatabase.parseDateValue(row["fileModifiedAt"]),
            status: status,
    ...
    static func sharedArtifactSyncState(from row: Row) -> SharedArtifactSyncStateRecord? {
        ...
        let createdAt = BurnBarDatabase.parseDateValue(row["createdAt"]) ?? Date()
        let updatedAt = BurnBarDatabase.parseDateValue(row["updatedAt"]) ?? createdAt
        ...
        return SharedArtifactSyncStateRecord(
            ...
            remoteUpdatedAt: BurnBarDatabase.parseDateValue(row["remoteUpdatedAt"]),
            lastPulledAt: BurnBarDatabase.parseDateValue(row["lastPulledAt"]),
            lastSyncedAt: BurnBarDatabase.parseDateValue(row["lastSyncedAt"]),
    ...
    static func sharedArtifactPermission(from row: Row) -> SharedArtifactPermissionRecord? {
        ...
        return SharedArtifactPermissionRecord(
            ...
            canRead: BurnBarDatabase.parseBoolValue(row["canRead"]) ?? true,
            canWrite: BurnBarDatabase.parseBoolValue(row["canWrite"]) ?? false,
            canShare: BurnBarDatabase.parseBoolValue(row["canShare"]) ?? false,
            createdAt: createdAt,
            updatedAt: updatedAt
        )
    }
    ...
    static func sharedArtifactAuditEvent(from row: Row) -> SharedArtifactAuditEventRecord? {
        ...
        let occurredAt = BurnBarDatabase.parseDateValue(row["occurredAt"]) ?? Date()
        let createdAt = BurnBarDatabase.parseDateValue(row["createdAt"]) ?? occurredAt
