...
final class ConversationStore {
    ...
    func upsertConversation(_ record: ConversationRecord) throws {
        let keyFilesJSON = try BurnBarDatabase.encodeJSON(record.keyFiles)
        let keyCommandsJSON = try BurnBarDatabase.encodeJSON(record.keyCommands)
        let keyToolsJSON = try BurnBarDatabase.encodeJSON(record.keyTools)
        ...
        try dbQueue.write { db in
            try db.execute(
                ...
                arguments: [
                    ...
                    BurnBarDatabase.encodeJSONStringArray(record.keyFiles),
                    BurnBarDatabase.encodeJSONStringArray(record.keyCommands),
                    BurnBarDatabase.encodeJSONStringArray(record.keyTools),
    ...
    func saveChatMessage(_ message: ChatMessageRecord, threadID: String) throws {
        ...
        if message.transcriptPieces.isEmpty {
            ...
            piecesJSON = try BurnBarDatabase.encodeTranscriptPieces(message.transcriptPieces)
        }
        ...
        return try dbQueue.read { db in
            ...
            return rows.compactMap { row -> ChatThreadSummary? in
                guard let id = row["threadID"] as? String,
                      let createdAt = BurnBarDatabase.parseDateValue(row["createdAt"]),
                      let updatedAt = BurnBarDatabase.parseDateValue(row["updatedAt"]) else {
                    return nil
                }

                let messageCount = row["messageCount"] as? Int
                    ?? Int((row["messageCount"] as? Int64) ?? 0)
                let lastMessageAt = BurnBarDatabase.parseDateValue(row["lastMessageAt"])
    ...
    static func conversation(from row: Row) -> ConversationRecord? {
        ...
        let keyFiles = BurnBarDatabase.decodeJSONStringArray(row["keyFiles"] as? String)
        let keyCommands = BurnBarDatabase.decodeJSONStringArray(row["keyCommands"] as? String)
        let keyTools = BurnBarDatabase.decodeJSONStringArray(row["keyTools"] as? String)
        ...
        let fileModifiedAt = BurnBarDatabase.parseDateValue(row["fileModifiedAt"])
        ...
        return ConversationRecord(
            ...
            summaryUpdatedAt: BurnBarDatabase.parseDateValue(row["summaryUpdatedAt"]),
    ...
    static func chatMessage(from row: Row) -> ChatMessageRecord? {
        guard let id = row["id"] as? String,
              ...
              let ts = BurnBarDatabase.parseDateValue(row["timestamp"]) else {
            return nil
        }

        let pieces = BurnBarDatabase.decodeTranscriptPieces(row["transcriptPiecesJSON"] as? String) ?? []
