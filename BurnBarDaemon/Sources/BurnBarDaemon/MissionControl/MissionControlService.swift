...
public actor BurnBarMissionControlService {
    ...
    public func notificationCommand(_ request: BurnBarNotificationCommandRequest) async throws -> BurnBarNotificationCommandResponse {
        ...
        case .runDaily, .runWeekly:
            ...
            let slug = request.arguments.first ?? (fallbackSummary.summary.activeProjectSlug ?? "burnbar")
    ...
    private func mergeProject(
        ...
        let summary = activityProject.summary.nonEmpty
            ...
            ?? existing?.summary
            ?? "Awaiting the next indexed BurnBar checkpoint."
    ...
    private func buildAutoTakeoverPrompt(
        ...
        return """
        BurnBar auto-takeover for mission \(mission.title) in project \(mission.projectSlug).
    ...
    private func buildMissionPacketPrompt(
        ...
        return """
        BurnBar mission execution for project \(mission.projectSlug).
    ...
    private func normalizedQuestion(
        ...
        return BurnBarPendingQuestionSnapshot(
            ...
            answerPlaceholder: question.answerPlaceholder?.nonEmpty ?? "Record the operator call BurnBar should carry forward…",
    ...
    private func deliverNewQuestionNotificationsIfNeeded(
        ...
        if config.local.isEnabled {
            do {
                try await transport.deliverLocalNotification("New BurnBar question", "\(title)\n\(body)")
    ...
    private func buildReviewPrompt(
        ...
        return """
        BurnBar \(cadence.rawValue) review for project \(project.displayName) (\(project.projectSlug)).
