...
BurnBar is now a daemon-first, local-first product.
...
BurnBar currently ships four coordinated control surfaces:
...
- the local CLI: `BurnBarCLI` entrypoints for daemon health, controller status, questions, followups, mission approval, and simulator replay

### Support tiers

| Tier | What it covers |
|------|----------------|
| **Core** | The four surfaces above plus shared `BurnBarCore` contracts; local SQLite + daemon-owned files remain canonical. |
| **Experimental** | Optional Firestore replication, iCloud file mirroring, Cursor connector + tunnel — opt-in and best-effort. |
| **Adjacent tooling** | Repo helper at `tools/burnbar-mcp/` (read-only MCP bridge to local SQLite) — optional for developers, not part of the runtime spine. |
| **Parked tests** | Additional files under `AgentLensTests/` that are **not** listed in `project.yml` for `BurnBarTests` — kept for future re-enablement; see `AgentLensTests/README.md`. |
...
BurnBar mission control is now a real runtime, not a projected shell.

It includes:

- real controller ingestion from BurnBar activity
...
- BurnBar remains local-first and daemon-first
...
The Xcode target **`BurnBarTests`** (declared in `project.yml`) compiles **only**:

- `AgentLensTests/AgentLensTests.swift`
- `AgentLensTests/BurnBarSearchIntegrationHarness.swift`

Other `AgentLensTests/**/*.swift` files remain in the tree but are **not** part of this bundle until they are re-added to `project.yml` and updated for current APIs. See `AgentLensTests/README.md` and `CONTRIBUTING.md` for the active vs parked policy.
